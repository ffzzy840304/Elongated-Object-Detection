...
def vis_detections(im, imagename, class_name, dets, thresh=0.9):
    """Visual debugging of detections."""
    for i in range(np.minimum(100, dets.shape[0])):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])
        score = dets[i, -1]
       # if class_name=='baseball_bat':
        if score > thresh: #and class_name=='baseball_bat':
             savename='/home/user01/faster-rcnn-pytorch/image_bbox/'+imagename[:-4]+'.txt'
             fsave=open(savename,'a+')
             fsave.writelines(class_name+' '+str(bbox[0])+' '+str(bbox[1])+' '+str(bbox[2])+' '+str(bbox[3])+' '+str(score))
             fsave.writelines('\n')
             fsave.close()
